...
export class ProviderClientMock extends ProviderClient {
  public async fetchAgents(): Promise<Agent[]> {
    const agentInfo = JSON.parse(JSON.stringify(this.mockFetch()));
    const agents = [];

    // const items:number = Number(agentInfo.pagination.totalItems || 0);
    // tslint:disable-next-line:no-console
    console.log("Items: " + agentInfo.pagination.totalItems);

    if (agentInfo.pagination.totalItems === "0") {
      // tslint:disable-next-line:no-console
